...
if [ "${TRAVIS_TAG}" != "" ]; then
  ...
  /usr/local/android-sdk/build-tools/25.0.0/zipalign -v 4 app/build/outputs/apk/app-release-unsigned.apk app/build/outputs/apk/app-release.apk
  ...
  echo "Only deploy to Play Store when a tag is present."
fi
